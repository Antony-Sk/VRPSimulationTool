...
interface CVRProblem {
    vehicles: VehicleType[];
    vertices: Coordinates[];
    demands: number[];
    edges: Edge[];
    depot_indices: number[];
    best_sol: number | null;
}
...
let vrpChart: Chart | null = null;
let currentProblem: CVRProblem | null = null;
let currentSolution: VRPSolution | null = null;
let resultsHistory: ResultEntry[] = [];

async function loadVRP(): Promise<void> {
    const fileSelect = document.getElementById('vrp-file') as HTMLSelectElement;
    const selectedFile = fileSelect.value;

    try {
        const response = await fetch(`/api/vrp/${selectedFile}`);
        if (!response.ok) throw new Error('Network response was not ok');

        currentProblem = await response.json() as CVRProblem;
...
async function solveVRP(): Promise<void> {
    if (!currentProblem) {
        alert('Please load a problem first');
        return;
    }

    const solverSelect = document.getElementById('vrp-solver') as HTMLSelectElement;
    const problemTypeSelect = document.getElementById('problem-type') as HTMLSelectElement;
    const fileSelect = document.getElementById('vrp-file') as HTMLSelectElement;
    ...
    try {
        ...
        currentSolution = await response.json() as VRPSolution;

        updateStats(
            currentSolution.distance.toFixed(2),
            currentSolution.visits.length
        );

        addResultToHistory(
            solverSelect.options[solverSelect.selectedIndex].text,
            fileSelect.options[fileSelect.selectedIndex].text,
            currentSolution.distance.toFixed(2),
            currentSolution.visits.length,
            problemTypeSelect.options[problemTypeSelect.selectedIndex].text,
            currentProblem.best_sol
        );
...
function updateStats(distance: string, routes: number): void {
    ...
    if (routesElement) routesElement.textContent = routes.toString();
}
...
function renderProblem(problem: CVRProblem): void {
    ...
    const ctx = (document.getElementById('vrpChart') as HTMLCanvasElement).getContext('2d');
    if (!ctx) return;
...
function renderSolution(problem: CVRProblem, solution: VRPSolution): void {
    if (!vrpChart) return;
...
async function fetchAvailableExamples(): Promise<void> {
    try {
        const response = await fetch('/examples');
        if (!response.ok) throw new Error('Failed to fetch examples');

        const examples = await response.json() as string[];
...
function populateExampleDropdown(examples: string[]): void {
    const dropdown = document.getElementById('vrp-file') as HTMLSelectElement;
    if (!dropdown) return;
